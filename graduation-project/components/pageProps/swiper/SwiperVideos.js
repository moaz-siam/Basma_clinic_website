...
export default function SwiperVideos({ videos, id, titletop }) {
  const [data, setData] = useState(videos ? videos : []); // لتخزين البيانات المسترجعة من الـ API
  const [loading, setLoading] = useState(videos ? false : true); // لتتبع حالة التحميل
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setData(result.videos); // تعيين البيانات المسترجعة
  ...
  }, [videos]); // [] عني أن الطلب سيحدث عند تحميل المكون فقط
    ...
    <div className="w-full h-full">
      <div className="mx-auto container my-[32px] relative px-4">
        <div className="flex justify-between items-center w-full mb-[30px]">
          <h3 className="text-[25px] font-[700]">{titletop}</h3>
          <div className="space-x-[8px]">
            <button
              className={`custom-prev-video-${id} bg-Basic text-white p-1 rounded-full cursor-pointer`}
            >
              <FaChevronRight size={20} />
            </button>
            <button
              className={`custom-next-video-${id} bg-Basic text-white p-1 rounded-full cursor-pointer`}
            >
        ...
        <Swiper
          ...
          navigation={{
            nextEl: `.custom-next-video-${id}`,
            prevEl: `.custom-prev-video-${id}`,
          }}
          pagination={{
            clickable: true,
            dynamicBullets: true,
            ...
            <SwiperSlide key={vid.id}>
              <div className="p-[16px] shadow-md rounded-2xl md:text-start text-center bg-white">
                <div className=" relative">
                  ...
                  <Image 
                    src={playVideo}
                    className="w-[40px] h-[40px] object-fill absolute top-[40%] left-[45%]"
                    alt="Error Img"/>
                </div>
                <h3 className="text-[18px] mb-[8px]">{vid.title}</h3>
                ...
                <Link
                  ...
                  href={`/resources/videos/${vid.title_en.endsWith('?') ? vid.title_en.slice(0 , -1).split(" ").join("-") : vid.title_en.split(" ").join("-")}/${vid.id}`}
                >
                  <p className="text-[16px] underline ">شاهد الان</p>
                  <FaArrowLeftLong />
                </Link>
              </div>
            </SwiperSlide>
